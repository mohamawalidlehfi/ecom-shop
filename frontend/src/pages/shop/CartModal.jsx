...
const handleQuantity = (type,id) => {
    const paylod ={type,id}
    dispatch(updateQuantity(paylod))

}
const handleRemove = (id) => {
    const paylod ={id}
    dispatch(removeFromCart(paylod))
}
                                ...
                                products.map((item, index) => (
                                        ...
                                        md:items-center md:justify-between shadow-md md:p-5 p-2 mb-4'>
                                            <div className='flex items-center'>
                                                <span className='mr-4 px-1 bg-colors-primary text-white 
                                                rounded-full'>0{index + 1}</span>
                                                <img src={item.image} alt=" " className='size-12 
                                                object-cover mr-4'/>
                                                <div>
                                                    <h5 className='text-lg font-medium'>{item.name}</h5>
                                                    <p className='text-gray-600 text-sm' >${Number
                                                ...
                                                justify-end items-center mt-2'>
                                                    <button
                                                        ...
                                                        text-gray-700 hover:bg-colors-primary hover:text-white ml-8'>-</button>
                                                    <span className='px-2 text-center mx-1'>{item.quantity}</span>
                                                    <button
                                                        onClick={ () => handleQuantity('increment', item.id)}
                                                        className='size-6 flex items-center 
                                                        justify-center px-1.5 rounded-full bg-gray-200
                                                        text-gray-700 hover:bg-colors-primary hover:text-white'>+</button>
                                                    <div className='ml-5'>
                                                        <button
                                                        onClick={() => handleRemove(item.id)}
                                                        className='text-red-400 hover:text-red-900 mr-4'
                                                        >Remove</button>
                                                    </div>
                                                </div>
                                            </div>
